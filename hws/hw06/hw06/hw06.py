    ...
    def vend(self):
        ...
        if self.stock > 0:
            if self.balance < self.price:
                toadd = self.price - self.balance
                print(f"'Please add ${toadd} more funds.'")
...
def two_list(vals, counts):
    ...
    def helper(vals, counts):
        if counts == 0:
            return Link.empty
        else:
            return Link(vals, helper(vals, counts - 1))

    head = helper(vals[0], counts[0])
    curr = head
    while curr.rest is not Link.empty:
        curr = curr.rest
    curr.rest = two_list(vals[1:], counts[1:])
    return head

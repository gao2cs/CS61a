...
def boar_brawl(player_score, opponent_score):
    ...
    points = 3 * abs(ones_digit - tens_digit)
    if points == 0:
        return 1
    else:
        return points
...
def is_always_roll(strategy, goal=GOAL):
    ...
    "*** YOUR CODE HERE ***"
    score1, score2 = 0, 0
    state = strategy(score1, score2)
    for s1 in range(0, goal):
        for s2 in range(0, goal):
            if strategy(s1, s2) != state:
                return False
            else:
                score1, score2 = s1, s2
                state = strategy(score1, score2)
...
def make_averaged(original_function, times_called=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def averaged_dice(num_rolls=None, dice=None):
        if num_rolls == None and dice == None:
            dice = original_function
            i, total = 1, 0
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    i, min_rolls_sofar, max_avg_scores_sofar = 1, 0, 0
    while i <= 10:
        averaged_dice = make_averaged(roll_dice, times_called)
        avg_score = averaged_dice(i, dice)

        if avg_score > max_avg_scores_sofar:
            max_avg_scores_sofar = avg_score
            min_rolls_sofar = i

        if avg_score == max_avg_scores_sofar:
            min_rolls_sofar = min(i, min_rolls_sofar)
        i += 1

    return min_rolls_sofar
    # END PROBLEM 9

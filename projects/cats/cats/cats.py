...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    p = [p for p in paragraphs if select(p) == True]
    if k > len(p) - 1:
        return ''
    else:
        return p[k]
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def isInPagraph(paragraph):
        p = remove_punctuation(paragraph).lower().split()
        for s in subject:
            if s in p: 
                return True
        return False
...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words), len(source_words))):
        ...
        ii, match_count = 0, 0
        while ii <= m - 1:
            if typed_word[ii] != source_word[ii]:
                break
            else:
                match_count += 1
                ii += 1
    ...
    return correct_count / len_typed_words * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if (typed_word in word_list) == True:
        return typed_word

    for i in range(len(word_list)):
        curr_word = word_list[i]
        diff = diff_function(typed_word, curr_word, limit)  
        if diff < min_diff:
            min_diff = diff
            min_index = i
...
def furry_fixes(typed, source, limit):
    ...
    def count_unmatch(typed, source, limit):
        if typed == '' or source == '':
        ...
        if typed[0] != source[0]:
            return count_unmatch(typed[1:], source[1:], limit - 1) + 1
        else:
            return count_unmatch(typed[1:], source[1:], limit)

    return count_unmatch(typed, source, limit)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    def count(typed, source):
        ...
        if typed[0] != source[0]:
            ...
            return 1 + count(typed[1:], source[1:])

    progress = count(typed, source) / len(source)
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        min_player_index, min_player_time = -1, float('inf')
        for j in player_indices:
            player_time = get_time(times, j, i)
            if player_time < min_player_time:
                min_player_index, min_player_time = j, player_time
            else:
                continue
        res[min_player_index].append(words[i])
    return res
    # END PROBLEM 10
